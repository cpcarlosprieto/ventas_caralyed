...
class PDF_Invoice extends FPDF
{
    ...
    function RoundedRect($x, $y, $w, $h, $r, $style = '')
    {
        ...
        $this->_Arc($xc - $r, $yc - $r * $MyArc, $xc - $r * $MyArc, $yc - $r, $xc - $r, $yc);
        $this->_out($op);
    }
    ...
    function addSociete($nom, $adresse, $logo, $ext_logo)
    {
        ...
        {
            die('Error: No se puede cargar la imagen del logo.');
        }

        $this->Image($logo, 5, 3, 25, 25);
        $this->SetXY($x1, $y1);
        $this->SetFont('Arial', 'B', 12);
        $this->Cell(0, 10, $nom, 0, 1, 'C');

        $this->SetFont('Arial', '', 10);
        $length = $this->GetStringWidth($adresse);
        $lignes = $this->sizeOfText($adresse, $length);
        $this->MultiCell(0, 6, $adresse, 0, 'C');
    }
...
function addTVAs($impuesto, $total_venta, $simbolo)
{
    ...
    // Subtotal
    $this->SetXY($re, $y1 + 5);
    $this->cellConSimbolo($simbolo, $total_venta / 1.18);

    // IGV
    $this->SetXY($rei, $y1 + 10);
    $this->cellConSimbolo($simbolo, $total_venta - ($total_venta / 1.18));

    // Total
    $this->SetXY($re, $y1 + 15);
    $this->cellConSimbolo($simbolo, $total_venta);
}

// Método auxiliar para imprimir un valor con su símbolo de moneda
function cellConSimbolo($simbolo, $valor)
{
    $lengthSimbolo = $this->GetStringWidth($simbolo);
    $this->Cell($lengthSimbolo, 2, $simbolo);

    $lengthValor = $this->GetStringWidth(round($valor, 2));
    $this->Cell($lengthValor, 2, round($valor, 2));
}
